...
/// <summary>
/// How a film is keyed across the mapping sources, so that all of them file one under the same
...
internal static class MovieKey
{
    ...
    /// <summary>
    /// The key for a TVDB film id, which TVDB numbers separately from its series.
    /// </summary>
    /// <param name="id">The id, as the provider wrote it.</param>
    /// <returns>The key, or <c>null</c> where that is not an id.</returns>
    ...
    /// <summary>
    /// The key an AniBridge film descriptor names, which is "&lt;provider&gt;_movie:&lt;id&gt;".
    /// </summary>
    /// <param name="descriptor">The descriptor as written.</param>
    /// <returns>The key, or <c>null</c> where the descriptor does not name a film this reads.</returns>
