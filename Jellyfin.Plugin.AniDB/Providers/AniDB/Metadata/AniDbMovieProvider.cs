...
public class AniDbMovieProvider(IApplicationPaths appPaths, ILogger<AniDbMovieProvider>? logger = null) : IRemoteMetadataProvider<Movie, MovieInfo>
{
    ...
    public async Task<MetadataResult<Movie>> GetMetadata(MovieInfo info, CancellationToken cancellationToken)
    {
        ...
        // An id another provider has already settled on names the film outright, where the name
        // is the weaker evidence: AniDB spells a great many titles differently, and a film shares
    ...
    /// <summary>
    /// Takes what an entry records about the one episode a film is, over what it records about
    /// itself.
    /// </summary>
    /// <remarks>
    /// A film AniDB registered in its own right is its entry's first ordinary episode, and the
    /// entry's own record is already the film's: there is nothing to take. The rest - a film
    ...
    /// alongside, and a library holding several such films of one show would show them as
    ...
    /// </para>
    /// </remarks>
    /// <param name="movie">The film, already filled from its entry.</param>
    ...
    private async Task ReadFromEpisode(Movie movie, MappedMovie mapped, string preferredMetadataLanguage)
    {
        ...
        {
            _logger.LogWarning(
                "{MovieName} is {EpisodeNumberInEntry} of AniDB anime {AnimeId}, where {Source} place it, but that entry records no such episode, so the film is described by the entry as a whole",
                movie.Name,
                mapped.Episode.Kind.Prefix() + mapped.Episode.Number,
                mapped.Episode.AnimeId,
                mapped.Source);
        ...
        // the film: the reader fills in what the document holds and says nothing about the rest,
        // and an episode with no summary of its own must not empty the film's.
